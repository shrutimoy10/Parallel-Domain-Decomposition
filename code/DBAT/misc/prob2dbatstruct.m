...
%fprintf("\nPROB: no of points: %d\n", nOP); --26321 object points(OP)

% Number of lens distortion coefficients.
nK=3; % radial distortion coefficients(k1,k2,k3)
nP=2; % tangential distortion coefficients(p1,p2)
ixK=5+(1:nK);
ixP=5+nK+(1:nP);

% Internal orientation.
IO=nan(5+nK+nP,nImages); % 10 x 60
IOstd=nan(size(IO));
IOcov=[];
...
if individualCameras
    ...
    % Block-invariant
    inner=repmat(prob.job.defCam,1,nImages);% 1x60 array containing the unique value defCam
...
IO(4,:)=aspect;
IO(5,:)=skew;
%IO block matrix constructed(10x60)
...
OP=nan(3,nOP); %3x26321
%OPstd=nan(3,nOP);
priorCP=nan(size(OP)); %3 x 26321
%fprintf("\nPROB: The size of OP is : %d x %d\n",size(priorCP,1),size(priorCP,2));
...
nMarkPts=nnz(ismember(imId(:,2),OPid)); %90561
%fprintf("\nPROB: the no of mark points are : %d\n",nMarkPts);
% Measured coordinates and visibility matrix.
markPts=nan(2,nMarkPts);
markStd=nan(2,nMarkPts);
IPid=nan(1,nMarkPts);
% Visibility matrix.
vis=logical(spalloc(nOP,nImages,nMarkPts)); % 26321 x 60
%fprintf("\nPROB: size of vis is  : %d x %d \n",size(vis,1),size(vis,2));
% Column "pointer".
colPos=spalloc(nOP,nImages,nMarkPts); % 26321 x 60
