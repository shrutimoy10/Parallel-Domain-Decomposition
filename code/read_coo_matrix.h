...
double** read_coo_matrix(char* block_type,int row,int col)
{
	...
	{
		if((fptr = fopen("/home/iiit/shrutimoy.das/Parallel-Domain-Decomposition/Results/Matrices/Hcs.txt","r")) == NULL)
		{
			printf("Error in opening Hcs.txt");
			exit(1);
		}
		fscanf(fptr,"%d[^\n]",&nnz);
		//printf("NNZ = %d\n", nnz); // 1589016
	...
	{
		if((fptr = fopen("/home/iiit/shrutimoy.das/Parallel-Domain-Decomposition/Results/Matrices/Hsc.txt","r")) == NULL)
		{
			printf("Error in opening Hsc.txt");
			exit(1);
		}
		fscanf(fptr,"%d[^\n]",&nnz);
		//printf("NNZ = %d\n", nnz); // 1589016
	...
	{
		if((fptr = fopen("/home/iiit/shrutimoy.das/Parallel-Domain-Decomposition/Results/Matrices/Hss.txt","r")) == NULL)
		{
			printf("Error in opening Hss.txt");
			exit(1);
		}
		fscanf(fptr,"%d[^\n]",&nnz);
		//printf("NNZ = %d\n", nnz); // 236889
...
//in the arrays row_idx[],col_idx[],val[]
double** read_file(char* input_filename)
{
	...
	r8st_header_read(input_filename, &row_min, &row_max, &col_min, &col_max, &row, &col, &nst );
